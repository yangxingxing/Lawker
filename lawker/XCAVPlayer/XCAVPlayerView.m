...
- (XZPlayProgressView *)progressView{
    if (!_progressView) {
        ...
        [_progressView.fullBtn2 addTarget:self action:@selector(fullBtnClicked2:) forControlEvents:UIControlEventTouchUpInside];
        [_progressView.fullBtn3 addTarget:self action:@selector(fullBtnClicked3:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)fullBtnClicked3:(UIButton *)sender{
    if (self.avPlayer.rate != 0) {
        [self pause];
        [self.topVideoTitleView resume];
...
//无法控制弹幕层
- (void)fullBtnClicked2:(UIButton *)sender{
    if (self.avPlayer.rate != 0) {
        [self.progressView.playBtn setImage:[UIImage imageNamed:@"on11"] forState:UIControlStateNormal];
    }else{
        [self.progressView.playBtn setImage:[UIImage imageNamed:@"on22"] forState:UIControlStateNormal];
    }
}

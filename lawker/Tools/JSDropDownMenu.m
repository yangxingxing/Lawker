...
@property (nonatomic, copy) NSArray *timg;
...
- (void)setDataSource:(id<JSDropDownMenuDataSource>)dataSource {
    ...
    NSMutableArray *tempBgimg = [[NSMutableArray alloc] initWithCapacity:_numOfMenu];
    
    for (int i = 0; i < _numOfMenu; i++) {
        //bgLayer
        CGPoint bgLayerPosition = CGPointMake((i+0.5)*bgLayerInterval, self.frame.size.height/2);
        
        UIImageView *menu = [[UIImageView alloc] initWithFrame:CGRectMake((i+0.5)*bgLayerInterval-35,10,82,30)];
        [menu setImage:[UIImage imageNamed:@"list3"]];
        [self addSubview:menu];
        [tempBgimg addObject:menu];
    ...
    //_bottomShadow.backgroundColor = [UIColor blackColor];
    _timg = [tempBgimg copy];
...
//    UILabel *titleLabel = (UILabel*)[cell viewWithTag:1];
    ...
    if (leftOrRight == 1) {
        ...
        if ([titleLabel.text isEqualToString:[(CATextLayer *)[_titles objectAtIndex:_currentSelectedMenudIndex] string]]) {
            ...
            UIImageView *accessoryImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"box"]];
            
            accessoryImageView.frame = CGRectMake(titleLabel.frame.origin.x+titleLabel.frame.size.width+10, (self.frame.size.height-15)/2, 16, 12);
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([cell.textLabel.text isEqualToString:[(CATextLayer *)[_titles objectAtIndex:_currentSelectedMenudIndex] string]]) {
        //cell.backgroundColor = BackColor;
        cell.accessoryView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"box"]];
    } else{

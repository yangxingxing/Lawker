...
@interface SXPay2iewController ()<UITableViewDataSource,UITableViewDelegate,WXApiDelegate>
{
    int payint;
    int payaddres;
    float payjf;
...
@implementation SXPay2iewController
...
- (void)loadDataForType:(int)type withURL:(NSString *)allUrlstring
{
    ...
    [[[SXNetworkTools sharedNetworkTools]GET:allUrlstring parameters:nil progress:nil success:^(NSURLSessionDataTask *task, NSDictionary* responseObject) {
        ...
        NSMutableArray *arrayM = [SXPay2Model objectArrayWithKeyValuesArray:temArray];
...
{
    SXPay2Model *newsModel = self.arrayList[indexPath.row];
    NSString *ID = [SXPay2Cell idForRow:newsModel];
    
    if ((indexPath.row%20 == 0)&&(indexPath.row != 0)) {
        ID = @"NewsCell";
    }
    SXPay2Cell * cell = [tableView dequeueReusableCellWithIdentifier:ID];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CGFloat rowHeight = [SXPay2Cell heightForRow:newsModel];
